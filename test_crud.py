...
h = r.json()
print(f"  Backend: {h['db_backend']}, Towers: {h['towers_loaded']}, Persisted: {h['towers_persisted']}")
...
r = requests.get(f"{BASE}/health")
h = r.json()
print(f"  Towers: {h['towers_loaded']}, Persisted: {h['towers_persisted']}")
check("counts match after cleanup", h["towers_loaded"] == h["towers_persisted"])
